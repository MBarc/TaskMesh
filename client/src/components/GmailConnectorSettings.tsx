...
export function GmailConnectorSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSettings(saved);
      setPassword('');
      setFeedback({ type: 'success', message: 'Gmail settings saved' });
    } catch (err: any) {
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              ...
              placeholder={settings?.passwordConfigured ? settings.passwordMasked : 'Enter app password'}
              className="w-full px-3 py-2 text-sm bg-surface border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 text-text-primary"
            />
          </div>
        </div>
